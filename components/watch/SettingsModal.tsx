...
import { Settings, Zap, Shield, Save, Gem, Filter, User, ShoppingCart, X } from "lucide-react"
import { ALL_POSSIBLE_INTERESTS, ALL_COUNTRIES } from "@/lib/constants"
import { Input } from "../ui/input"
import Link from "next/link"
import { createClient } from "@/lib/supabase/client"
import { CountrySelectionModal } from "./CountrySelectionModal" // Import the new modal
...
export function SettingsModal({ isOpen, onOpenChange, onSave, isInitialSetup }: SettingsModalProps) {
  ...
  // State to control the visibility of the country selection modal
  ...
  const userCountry = React.useMemo(() => {
    ...
    return ALL_COUNTRIES.find((c) => c.name === profile.country)
  }, [profile?.country])
  ...
  return (
    <>
      <Dialog open={isOpen} onOpenChange={isInitialSetup ? () => {} : onOpenChange}>
        <DialogContent className="bg-black/30 backdrop-blur-xl border-gray-700 text-white max-w-[95%] sm:max-w-2xl p-0 flex flex-col">
                  ...
                  <div className="space-y-6 p-2">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                        ...
                        <div className="space-y-2">
                          <Label htmlFor="dob" className="font-semibold text-gray-400">
                            Date of Birth
                          </Label>
                          {isInitialSetup ? (
                            <Input
                            ...
                            <p className="p-2 bg-gray-900/50 rounded-md h-10 flex items-center">
                              {dob ? new Date(dob).toLocaleDateString() : "Not set"}
                            </p>
                          )}
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor="gender" className="font-semibold text-gray-400">
                            Your Gender
                          </Label>
                          {isInitialSetup ? (
                            <Select
                              value={gender || ""}
                              onValueChange={(value) => setGender(value as "male" | "female" | "couple")}
                            >
                            ...
                            <p className="p-2 bg-gray-900/50 rounded-md h-10 flex items-center capitalize">
                              {gender || "Not set"}
                            </p>
                          )}
                        </div>
                        <div className="space-y-2">
                          ...
                          <div className="p-2 bg-gray-900/50 rounded-md h-10 flex items-center gap-2">
                            {userCountry ? (
                              <img
                    ...
                    <div className={isInitialSetup ? "hidden" : "space-y-6"}>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <Switch
                          id="show-vip-badge"
                          checked={settings.show_vip_badge}
                          onCheckedChange={(checked) => handleSettingChange("show_vip_badge", checked)}
                          disabled={!isVip}
                        />
                      </div>
                    </div>
                ...
                <TabsContent value="matching" className={isInitialSetup ? "hidden" : ""}>
                  <div className="space-y-6 p-2">
                    <div className="flex items-center justify-between">
                      ...
                      <Switch
                        id="auto-roll"
                        checked={settings.auto_roll}
                        onCheckedChange={(checked) => handleSettingChange("auto_roll", checked)}
                      />
                    </div>
                    <div>
                      ...
                      >
                        {ALL_POSSIBLE_INTERESTS.map((interest) => (
                          <ToggleGroupItem
                            key={interest.name}
                            value={interest.name}
                            className="border-gray-600 hover:bg-gray-700 data-[state=on]:bg-purple-600 data-[state=on]:text-white"
                          >
                    ...
                    <div>
                      <Label htmlFor="interest-wait" className="font-semibold">
                        Interests Max Wait ({settings.interest_max_wait_time}s)
                      </Label>
                      <p className="text-xs text-gray-400 mb-2">
                        Time to search for interests before matching with anyone.
                      </p>
                      <Slider
                        id="interest-wait"
                        value={[settings.interest_max_wait_time]}
                        max={60}
                        step={5}
                        onValueChange={(value) => handleSettingChange("interest_max_wait_time", value[0])}
                      />
                    </div>
                  </div>
                </TabsContent>
                ...
                <TabsContent value="filters" className={isInitialSetup ? "hidden" : ""}>
                  <div className="space-y-6 p-2">
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mb-2">
                        Select countries to match with. Leave empty to match with anyone.
                      </p>

                      {/* Button to open the new CountrySelectionModal */}
                      <Button
                        variant="outline"
                        className="w-full justify-start text-left font-normal bg-gray-800 border-gray-600 hover:bg-gray-700"
                        onClick={() => setIsCountryModalOpen(true)}
                      >
                        {settings.preferred_countries.length > 0
                          ? `${settings.preferred_countries.length} countries selected`
                          : "Select countries..."}
                      </Button>

                      <div className="flex flex-wrap gap-2 pt-3">
                        {settings.preferred_countries.map((countryName) => {
                          const country = ALL_COUNTRIES.find((c) => c.name === countryName)
                          return (
                            <Badge
                              key={countryName}
                              variant="secondary"
                              className="font-normal bg-gray-700 text-gray-200 flex items-center gap-1 pl-2 pr-1"
                            >
                              {country && (
                                <img
                              ...
                              <button
                                onClick={() => {
                                  const newSelection = settings.preferred_countries.filter(
                                    (c) => c !== countryName
                                  )
                                  handleSettingChange("preferred_countries", newSelection)
                                }}
                                className="ml-1 rounded-full hover:bg-gray-600/50 p-0.5"
                    ...
                    <div>
                      <Label htmlFor="country-wait" className="font-semibold">
                        Country Max Wait ({settings.country_max_wait_time}s)
                      </Label>
                      <p className="text-xs text-gray-400 mb-2">
                        Time to search for countries before matching with anyone.
                      </p>
                      <Slider
                        id="country-wait"
                        value={[settings.country_max_wait_time]}
                        max={60}
                        step={5}
                        onValueChange={(value) => handleSettingChange("country_max_wait_time", value[0])}
                      />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="preferred-gender" className="font-semibold">
                        Preferred Gender
                      </Label>
                      <Select
                        value={settings.preferred_gender}
                        onValueChange={(value) => handleSettingChange("preferred_gender", value)}
                        disabled={!canFilterByGender}
                      >
                        <SelectTrigger className="w-full bg-gray-800 border-gray-600">
                          <SelectValue
                            placeholder={canFilterByGender ? "Select preferred gender" : "VIP/Boost Feature"}
                          />
                        </SelectTrigger>
                        <SelectContent className="bg-gray-800 border-gray-700 text-white">
                      ...
                      {!canFilterByGender && (
                        <p className="text-xs text-amber-400 mt-1">Upgrade to VIP or Boost to use this feature.</p>
                      )}
                    </div>
                  </div>
                </TabsContent>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                      ...
                      {profile && profile.boosts > 0 ? (
                        <div className="bg-gray-900/50 p-4 rounded-lg">
                          ...
                          <Button onClick={handleActivateBoost} className="mt-4 w-full bg-purple-600 hover:bg-purple-700">
                            Activate Boost
                          </Button>
                        </div>
                      ) : (
                      ...
                      {isVip ? (
                        <div className="bg-gray-900/50 p-4 rounded-lg">
                          ...
                          <p className="text-xs text-gray-500">
                            Expires: {profile.vip_until ? new Date(profile.vip_until).toLocaleDateString() : "N/A"}
                          </p>
                          <Button
                        ...
                        <div className="bg-gray-900/50 p-4 rounded-lg">
                          ...
                          <Link href="/subscription">
                            <Button className="mt-4 w-full bg-yellow-500 hover:bg-yellow-600 text-black">
                              Become a VIP
                            </Button>
                          </Link>
                        </div>
      ...
      {/* Render the new CountrySelectionModal */}
      <CountrySelectionModal
        isOpen={isCountryModalOpen}
        onOpenChange={setIsCountryModalOpen}
        selectedCountries={settings.preferred_countries}
        onCountriesChange={(countries) => handleSettingChange("preferred_countries", countries)}
      />
    </>
  )
}
